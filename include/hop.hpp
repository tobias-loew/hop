...
*/
    ...
    // applying each element of tuple to function

    namespace impl {
        template <class Tuple, class F, std::size_t... I>
        constexpr F tuple_for_each_helper(Tuple&& t, F&& f, std::index_sequence<I...>)
        {
            return (void)std::initializer_list<int>{(std::forward<F>(f)(std::get<I>(std::forward<Tuple>(t))), 0)...}, f;
        }
    }
    template <class Tuple, class F>
    constexpr F tuple_for_each(Tuple&& t, F&& f)
    {
        return impl::tuple_for_each_helper(std::forward<Tuple>(t), std::forward<F>(f),
